...
public final class CaptureActivityHandler extends Handler {
    ...
    public void handleMessage(Message message) {
        Bitmap bitmap;
        switch (message.what) {
            case R.id.auto_focus /*2131165269*/:
                if (this.state == State.PREVIEW) {
                    CameraManager.get().requestAutoFocus(this, R.id.auto_focus);
                    return;
                }
                return;
            case R.id.decode_failed /*2131165406*/:
                this.state = State.PREVIEW;
                CameraManager.get().requestPreviewFrame(this.decodeThread.getHandler(), R.id.decode);
                return;
            case R.id.decode_succeeded /*2131165407*/:
                ...
                return;
            case R.id.launch_product_query /*2131165570*/:
                Log.d(TAG, "Got product query message");
                Intent intent = new Intent("android.intent.action.VIEW", Uri.parse((String) message.obj));
                intent.addFlags(524288);
                this.activity.startActivity(intent);
                return;
            case R.id.restart_preview /*2131165724*/:
                Log.d(TAG, "Got restart preview message");
                restartPreviewAndDecode();
                return;
            case R.id.return_scan_result /*2131165725*/:
