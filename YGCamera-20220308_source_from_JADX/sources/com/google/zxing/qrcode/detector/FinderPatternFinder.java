        ...
        if (size >= 3) {
            if (size > 3) {
                ...
                for (FinderPattern estimatedModuleSize : this.possibleCenters) {
                    ...
                    double d3 = (double) estimatedModuleSize2;
                    Double.isNaN(d3);
                    d += d3;
                    double d4 = (double) (estimatedModuleSize2 * estimatedModuleSize2);
                    Double.isNaN(d4);
                    d2 += d4;
                }
                ...
                double d6 = d / d5;
                Double.isNaN(d5);
                float sqrt = (float) Math.sqrt((d2 / d5) - (d6 * d6));
                float f = (float) d6;
                Collections.sort(this.possibleCenters, new FurthestFromAverageComparator(f, (FurthestFromAverageComparator) null));
                float max = Math.max(f * 0.2f, sqrt);
                int i = 0;
                while (i < this.possibleCenters.size() && this.possibleCenters.size() > 3) {
                    double estimatedModuleSize3 = (double) this.possibleCenters.get(i).getEstimatedModuleSize();
                    Double.isNaN(estimatedModuleSize3);
                    if (Math.abs(estimatedModuleSize3 - d6) > ((double) max)) {
            ...
            if (this.possibleCenters.size() > 3) {
                float f2 = 0.0f;
                for (FinderPattern estimatedModuleSize4 : this.possibleCenters) {
                    f2 += estimatedModuleSize4.getEstimatedModuleSize();
                }
                Collections.sort(this.possibleCenters, new CenterComparator(f2 / ((float) this.possibleCenters.size()), (CenterComparator) null));
