...
public final class CacheInterceptor implements Interceptor {
    ...
    public Response intercept(Interceptor.Chain chain) throws IOException {
        ...
        if (request == null && response2 == null) {
            return new Response.Builder().request(chain.request()).protocol(Protocol.HTTP_1_1).code(c.COLLECT_MODE_TIKTOK_GUEST).message("Unsatisfiable Request (only-if-cached)").body(Util.EMPTY_RESPONSE).sentRequestAtMillis(-1).receivedResponseAtMillis(System.currentTimeMillis()).build();
        }
        if (request == null) {
    ...
    private Response cacheWritingResponse(final CacheRequest cacheRequest, Response response) throws IOException {
        ...
        return response.newBuilder().body(new RealResponseBody(response.header(com.alibaba.sdk.android.oss.common.utils.HttpHeaders.CONTENT_TYPE), response.body().contentLength(), Okio.buffer((Source) r2))).build();
    }
    ...
    static boolean isContentSpecificHeader(String str) {
        return com.alibaba.sdk.android.oss.common.utils.HttpHeaders.CONTENT_LENGTH.equalsIgnoreCase(str) || com.alibaba.sdk.android.oss.common.utils.HttpHeaders.CONTENT_ENCODING.equalsIgnoreCase(str) || com.alibaba.sdk.android.oss.common.utils.HttpHeaders.CONTENT_TYPE.equalsIgnoreCase(str);
    }
}
