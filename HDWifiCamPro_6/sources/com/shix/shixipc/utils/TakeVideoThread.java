        ...
        while (true) {
            ...
            if (this.video_i == 1) {
                ...
                if (this.tag > 0) {
                    ...
                    File file = new File(sDFilePath, "HDWiFiCam/video/" + this.strDID);
                    if (!file.exists()) {
                        file.mkdirs();
                    }
                    String str = this.szDid;
                    NativeCaller.OpenAviFileName(str, CommonUtil.getSDFilePath() + "/" + ContentCommon.SDCARD_PATH + "/video/" + this.strDID + "/" + this.name, "h264", this.height, this.width, this.frame, 8000);
                    this.widthRun = this.width;
                    this.heightRun = this.height;
                    CommonUtil.Log(1, "ZHAOTAKEVIDEO h264");
                } else {
                    File sDFilePath2 = CommonUtil.getSDFilePath();
                    File file2 = new File(sDFilePath2, "HDWiFiCam/video/" + this.strDID);
                    if (!file2.exists()) {
                        file2.mkdirs();
                    }
                    String str2 = this.szDid;
                    NativeCaller.OpenAviFileName(str2, CommonUtil.getSDFilePath() + "/" + ContentCommon.SDCARD_PATH + "/video/" + this.strDID + "/" + this.name, "mjpg", this.height, this.width, this.frame, 8000);
                    CommonUtil.Log(1, "ZHAOTAKEVIDEO mjpg");
                    this.widthRun = this.width;
                    this.heightRun = this.height;
                }
            }
            ...
            if (i > 1 && i < 120000 / this.sleepTime) {
                if (this.widthRun == this.width && this.heightRun == this.height) {
                    ...
                    if (this.tag != 0) {
                        ...
                        Log.e(ContentCommon.SERVER_STRING, "kkkkkWriteVideoData width:" + this.width + "  heigh:" + this.height + "videoByte.length:" + this.videoByte.length);
                    } else {
                        if (this.videoByte == null) {
                            this.videoByte = new byte[0];
                        }
                    ...
                    this.videoBuffer.ClearAll();
                    if (this.video_i < 20) {
                        boolean delete = new File(CommonUtil.getSDFilePath() + "/" + ContentCommon.SDCARD_PATH + "/video/" + this.strDID + "/" + this.name).delete();
