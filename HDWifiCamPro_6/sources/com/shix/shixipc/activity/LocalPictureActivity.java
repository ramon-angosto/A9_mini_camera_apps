...
public class LocalPictureActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void initView() {
        ...
        initTopView();
        if (this.isVideo) {
            file = new File(CommonUtil.getSDFilePath().getAbsolutePath() + "/" + ContentCommon.SDCARD_PATH + "/videohead/" + this.uid + "/");
        } else {
            file = new File(CommonUtil.getSDFilePath().getAbsolutePath() + "/" + ContentCommon.SDCARD_PATH + "/Snapshot/" + this.uid + "/");
        }
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long j) {
                ...
                if (i >= 0) {
                    if (LocalPictureActivity.this.isDeleteModel) {
                        ...
                        Advertisement.FILE_SCHEME + replace;
                        ...
                        if (!file.exists() || replace.indexOf("avi") <= 0) {
                            ...
                            Intent intent2 = new Intent(LocalPictureActivity.this, ShowLocalVideoActivity.class);
                            intent2.putExtra("filepath", replace2);
                            intent2.putExtra(ContentCommon.STR_CAMERA_NAME, LocalPictureActivity.this.strName);
                            LocalPictureActivity.this.startActivity(intent2);
                            return;
                        }
                        if (Build.VERSION.SDK_INT >= 24) {
                            uri = FileProvider.getUriForFile(LocalPictureActivity.this, LocalPictureActivity.this.getPackageName() + ".fileProvider", file);
                        } else {
                        ...
                        intent3.setClass(LocalPictureActivity.this, ImagePagerActivity.class);
                        intent3.putExtra(ContentCommon.STR_CAMERA_ID, LocalPictureActivity.this.uid);
                        intent3.putParcelableArrayListExtra("girdlst", LocalPictureActivity.this.mGirdList);
                        intent3.putExtra("position", i);
                        LocalPictureActivity.this.startActivityForResult(intent3, 10);
                    }
                }
            }
        });
