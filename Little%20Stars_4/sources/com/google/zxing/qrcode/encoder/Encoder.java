...
public final class Encoder {
    ...
    public static void encode(String str, ErrorCorrectionLevel errorCorrectionLevel, QRCode qRCode) throws WriterException {
        encode(str, errorCorrectionLevel, (Map<EncodeHintType, ?>) null, qRCode);
    }
    ...
    private static int getTotalInputBytes(int i, Version version, Mode mode) {
        return ((i + (mode.getCharacterCountBits(version) + 4)) + 7) / 8;
    }
                ...
                for (int i12 = 0; i12 < i8; i12++) {
                    for (BlockPair dataBytes : arrayList) {
                        ...
                        if (i12 < dataBytes2.length) {
                            bitArray3.appendBits(dataBytes2[i12], 8);
                        }
                    }
                }
                for (int i13 = 0; i13 < i9; i13++) {
                    for (BlockPair errorCorrectionBytes : arrayList) {
                        ...
                        if (i13 < errorCorrectionBytes2.length) {
                            bitArray3.appendBits(errorCorrectionBytes2[i13], 8);
                        }
                    }
                }
